...
export default function (
  ...
  if (!token) {
    return next(
      new AppError(ERROR_MESSAGES.INVALID_AUTH_TOKEN, 401),
    );
  }
  try {
    ...
    const userId = (
      decodedToken as { [key: string]: string }
    )["id"];
    const userEmail = (
      decodedToken as { [key: string]: string }
    )["email"];
    const userRole = (
      decodedToken as { [key: string]: USER_ROLES }
    )["role"];
    if (!userRole) {
      throw new AppError(
        ERROR_MESSAGES.INVALID_AUTH_TOKEN,
        401,
      );
    }
...
export async function validateNewPassword(
  ...
  const isChangingPassword = req.path.endsWith(
    "/change_password",
  );
  ...
  if (
    ...
    return next(
      new AppError(
        ERROR_MESSAGES.REQUEST_BODY_MISSING,
        400,
      ),
    );
  }
  ...
  if (
    ...
    isPasswordInRequestCorrect =
      await comparePasswordWithActual<CustomerModel>(
        CustomerModel,
        id,
        currentPassword,
      );
  ...
  if (
    ...
    isPasswordInRequestCorrect =
      await comparePasswordWithActual<StaffModel>(
        StaffModel,
        id,
        currentPassword,
      );
  ...
  if (!isPasswordInRequestCorrect) {
    return next(
      new AppError(
        ERROR_MESSAGES.RESET_PASSWORD_CURRENT_ACTUAL_MISMATCH,
        400,
      ),
    );
  }
  ...
  if (newPassword !== confirmedNewPassword) {
    return next(
      new AppError(
        ERROR_MESSAGES.RESET_PASSWORD_MISMATCH,
        400,
      ),
    );
  }
  ...
  if (
    ...
    String(confirmedNewPassword).length < 8
  ) {
    return next(
      new AppError(
        ERROR_MESSAGES.PASSWORD_LENGTH_NOT_MET,
        400,
      ),
    );
  }
