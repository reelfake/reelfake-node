...
describe('Movie Languages Controller', () => {
  it('GET /api/v1/movie_languages should return all available languages', async () => {
    const server = supertest(app);
    const response = await server.get('/api/v1/movie_languages').set('api-key', apiKey);
  ...
  it('GET /api/v1/movie_languages should return 500 on exception', async () => {
    jest
